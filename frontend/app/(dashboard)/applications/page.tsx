...
export default function ApplicationsPage() {
  ...
  const fetch = async (p: number, s: string, q: string) => {
    setLoading(true);
    try {
  ...
  const handleSearch = () => { setPage(1); fetch(1, statusFilter, search); };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">درخواست‌ها</h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700/50">
              ...
              <th className="px-6 py-4 text-right text-xs font-semibold text-slate-400">منبع</th>
