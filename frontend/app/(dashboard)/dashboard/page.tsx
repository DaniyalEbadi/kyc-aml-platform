    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">سلام، خوش آمدید</p>
        </div>
        ...
        <ChartCard title="سند بر اساس نوع">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data.document_type_distribution.filter(d => d.value > 0)}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis dataKey="label" stroke="#64748b" fontSize={11} tickFormatter={(v) => {
                const docTypeMap: Record<string, string> = { passport: "گذرنامه", national_id: "کارت ملی", driver_license: "گواهینامه", proof_of_address: "نشانی", selfie: "سلفی", business: "کسب‌وکار", residence: "اقامت" };
                return docTypeMap[v] || v;
              }} />
              <YAxis stroke="#64748b" fontSize={11} />
              <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "8px", direction: "rtl" }} />
